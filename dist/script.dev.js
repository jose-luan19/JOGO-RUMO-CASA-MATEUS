...
var feedbackTimer;
var transitionLock = false; // ===== PERGUNTAS =====
...
function checkAnswer(selected) {
  ...
  if (selected === levels[level].answer) {
    transitionLock = true;
    showFeedback("✅ Resposta certa!", 2000, function () {
      level++;
      updateBackButton();
...
function drawGround() {
  ...
  ctx.fillRect(0, 230, canvas.width, 40); // Grama

  ctx.fillStyle = "#4CAF50";
  ctx.fillRect(0, 220, canvas.width, 10); // Linha de separação (textura fake)
